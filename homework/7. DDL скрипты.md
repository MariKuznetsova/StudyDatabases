...
CREATE TABLE if not exists catalog.PublisherSeries (
        ...
        PublisherSeriesName varchar (1000) NOT NULL);
...
CREATE TABLE if not exists catalog.WorkSeries (
        ...
        TotalNumber smallint NOT NULL CHECK (TotalNumber > 0));
...
CREATE TYPE catalog.covertype AS ENUM ('Мягкая обложка', 'Твердая обложка');
...
CREATE TABLE if not exists catalog.Book (
        ...
        PageCounts smallint NOT NULL CHECK (PageCounts > 0),
...
CREATE TYPE catalog.specialistrole AS ENUM ('Серийное оформление', 'Оформление обложки', 'Автор иллюстраций', 'Ответственный редактор', 'Научный редактор', 'Корректор', 'Компьютерная верстка');

CREATE TABLE if not exists catalog.SpecialistBook (
        SpecialistID integer,
        ISBN char (17),
        Role catalog.specialistrole NOT NULL,
        PRIMARY KEY (SpecialistID, ISBN),
...
CREATE TABLE if not exists accounting.PrintOrder (
        ...
        SheetCount numeric (5,2) NOT NULL CHECK (SheetCount > 0),
        PaperType accounting.papertypetype NOT NULL,
        Printing accounting.printingtype NOT NULL,
        СopiesCount integer NOT NULL CHECK (СopiesCount > 0),
        TotalPrice numeric (12,4) NOT NULL CHECK (TotalPrice > 0),
...
CREATE TABLE if not exists accounting.Order (
        ...
        TotalPrice numeric (14,4) NOT NULL CHECK(TotalPrice > 0),
        Status accounting.statusorder NOT NULL,
        CustomerID integer NOT NULL,
        TotalCount integer NOT NULL CHECK(TotalCount > 0),
        TotalWeight integer NOT NULL CHECK(TotalWeight > 0),
...
CREATE TABLE if not exists accounting.BookOrder (
        ISBN char (17),
        OrderID integer,
        BookPrice numeric (10,4) NOT NULL CHECK (BookPrice > 0),
